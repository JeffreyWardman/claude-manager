...
import { sortActiveFirst, sortByDate, sortAlpha, sortSessions, groupByStatus, groupByLocation, projectLabel, containsMatch, sessionMatchesSearch, parseIgnorePatterns, isSessionIgnored } from "./sidebarUtils";
...
describe("sortSessions", () => {
  it("delegates to the correct sort function", () => {
    const sessions = [
      makeSession({ session_id: "off", status: "offline", started_at: 100, display_name: "AAA" }),
      makeSession({ session_id: "act", status: "active", started_at: 50, display_name: "ZZZ" }),
    ];
...
describe("groupByLocation", () => {
  ...
  it("sorts active sessions above offline within a location group", () => {
    const sessions = [
      makeSession({ session_id: "off", status: "offline", cwd: "/repos/app", started_at: 100 }),
      makeSession({ session_id: "act", status: "active", cwd: "/repos/app", started_at: 50 }),
    ];
    const groups = groupByLocation(sessions);
  ...
  it("sorts location groups with active sessions above all-offline groups", () => {
    const sessions = [
      makeSession({ session_id: "off", status: "offline", cwd: "/repos/alpha" }),
  ...
  it("active group order is maintained when session status changes", () => {
    ...
    const before = [
      makeSession({ session_id: "s1", status: "offline", cwd: "/repos/app", started_at: 10 }),
      makeSession({ session_id: "s2", status: "active", cwd: "/repos/app", started_at: 5 }),
    ];
    ...
    // Both active now
    const after = [
      makeSession({ session_id: "s1", status: "active", cwd: "/repos/app", started_at: 10 }),
      makeSession({ session_id: "s2", status: "active", cwd: "/repos/app", started_at: 5 }),
    ];
...
describe("sessionMatchesSearch", () => {
  it("matches against display name", () => {
    const s = makeSession({ session_id: "abc-123", display_name: "My Session" });
    expect(sessionMatchesSearch(s, "my ses")).toBe(true);
  });

  it("matches against project name when no display name", () => {
    const s = makeSession({ session_id: "abc-123", project_name: "backend-api" });
    expect(sessionMatchesSearch(s, "backend")).toBe(true);
  });

  it("matches against cwd", () => {
    const s = makeSession({ session_id: "abc-123", cwd: "/home/user/repos/frontend" });
  ...
  it("matches cwd with tilde form", () => {
    const s = makeSession({ session_id: "abc", cwd: "/Users/jeffrey/repos/frontend" });
  ...
  it("returns false when nothing matches", () => {
    const s = makeSession({ session_id: "abc", project_name: "app", cwd: "/tmp" });
...
describe("isSessionIgnored", () => {
  ...
  it("does not ignore subfolders of a path pattern", () => {
    const s = makeSession({ session_id: "a", cwd: "/Users/user/repos/project" });
  ...
  it("un-ignores with ! pattern", () => {
    const s = makeSession({ session_id: "a", cwd: "/Users/user/tmp/important" });
    const patterns = parseIgnorePatterns("tmp/\n!tmp/important");
    expect(isSessionIgnored(s, patterns)).toBe(false);
  });
  ...
  it("matches glob * pattern in path", () => {
    const s = makeSession({ session_id: "a", cwd: "/Users/user/repos/tmp-project" });
    const patterns = parseIgnorePatterns("repos/tmp-*");
    expect(isSessionIgnored(s, patterns)).toBe(true);
  });

  it("glob * does not match across /", () => {
    const s = makeSession({ session_id: "a", cwd: "/Users/user/repos/tmp/nested" });
    const patterns = parseIgnorePatterns("repos/tmp*nested");
    expect(isSessionIgnored(s, patterns)).toBe(false);
  });

  it("glob ** matches across /", () => {
    const s = makeSession({ session_id: "a", cwd: "/Users/user/repos/deep/nested/tmp" });
    const patterns = parseIgnorePatterns("repos/**/tmp");
    expect(isSessionIgnored(s, patterns)).toBe(true);
  });
  ...
  it("un-ignores with glob pattern", () => {
    const s = makeSession({ session_id: "a", display_name: "test.env.example" });
