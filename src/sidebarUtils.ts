...
export function sortSessions(sessions: ClaudeSession[], mode: SortMode): ClaudeSession[] {
  switch (mode) {
    case "active": return sortActiveFirst(sessions);
    case "date": return sortByDate(sessions);
    case "alpha": return sortAlpha(sessions);
  }
}
...
export function groupByStatus(sessions: ClaudeSession[], sort: SortMode = "active"): SidebarGroup[] {
  return [
    { label: "ACTIVE", sessions: sortSessions(sessions.filter((s) => s.status === "active"), sort) },
    { label: "OFFLINE", sessions: sortSessions(sessions.filter((s) => s.status === "offline"), sort) },
  ].filter((g) => g.sessions.length > 0);
}

export function groupByLocation(sessions: ClaudeSession[], sort: SortMode = "active"): SidebarGroup[] {
...
export function parseIgnorePatterns(raw: string): { include: string[]; exclude: string[] } {
...
function patternMatchesSession(
  ...
  if (/[*?]/.test(pattern)) {
    return matchesGlob(name, pattern) || matchesGlob(cwd, pattern) || matchesGlob(cwdTilde, pattern);
  }
...
export function isSessionIgnored(session: ClaudeSession, patterns: { include: string[]; exclude: string[] }): boolean {
...
export function sessionMatchesSearch(session: ClaudeSession, query: string): boolean {
  ...
  if (containsMatch(name, query) || containsMatch(session.session_id, query)) return true;
