...
export function MainPane({ session, gridSlotIdx, onGridClose, activityMap, unreadSessions, focused }: Props) {
  ...
  if (!session) {
    return (
      ...
      >
        ...
        <div style={{ fontSize: 11, marginTop: 4, color: "var(--text-very-muted)", pointerEvents: "none" }}>
  ...
  return (
    <div className="flex flex-col flex-1 h-full" style={{ background: "var(--bg-main)" }}>
      ...
      >
        ...
        {/* View tabs */}
        <div role="tablist" aria-label="View mode" style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 4 }}>
          <button
            role="tab"
          ...
          <span aria-hidden="true" style={{ color: "var(--text-very-muted)", fontSize: 10 }}>|</span>
          <button
            role="tab"
